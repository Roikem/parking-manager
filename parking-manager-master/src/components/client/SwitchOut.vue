<template>
  <div>
    ...
    <el-card>
      ...
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <el-form-item label="车位类型">
          <el-select v-model="formInline.status" placeholder="请选车位类型">
    ...
    <el-dialog title="车位类型切换 " :visible.sync="reletVisible" width="30%">
...
export default {
  ...
  methods: {
    ...
    fetch() {
      ...
      listData.status = this.formInline.status;
    ...
    searchForm() {
      var searchForm = {};
      searchForm.status = this.formInline.status;
    ...
    filterHandler(value, row, column) {
      if (this.filter) {
        if (this.filter == value) {
          ...
          this.filter = value;
          console.log("1现在所选择的是" + this.filter);
          var filterForm = {};
          filterForm.status = this.filter;
          filterForm.locate = this.formInline.locate;
          filterForm.pageNum = this.queryInfo.pagenum;
          filterForm.pageSize = 10;
          let listSearch = this.$qs.stringify(filterForm);
          console.log(listSearch);
          this.openLoading();
          this.$http
            .get(this.api + "SwitchInfo?" + listSearch)
        ...
        this.filter = value;
        console.log("2现在所选择的是" + this.filter);
        var searchForm = {};
        searchForm.status = this.filter;
        searchForm.locate = this.formInline.locate;
        searchForm.pageNum = this.queryInfo.pagenum;
        searchForm.pageSize = 10;
        let listSearch = this.$qs.stringify(searchForm);
        console.log(listSearch);
        this.openLoading();
